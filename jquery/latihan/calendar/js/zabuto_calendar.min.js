...
$.fn.zabuto_calendar = function(b) {
    ...
    this.each(function() {
        ...
        function l() {
            ...
            if (false !== A) { r(j, C.getFullYear(), C.getMonth()) } }
        ...
        function g(A) {
            ...
            if (typeof(z) == "object" && z.length > 0) { $(z).each(function(E, G) {
                    if (typeof(G) == "object") {
                        if ("type" in G) {
                            var F = "";
                            if ("label" in G) { F = G.label }
                            switch (G.type) {
                                case "text":
                                    if (F !== "") {
                                        var D = "";
                                ...
                                case "list":
                                    if ("list" in G && typeof(G.list) == "object" && G.list.length > 0) {
                                        ...
                                        $(G.list).each(function(J, I) { B.append('<li class="' + I + '"></li>') });
                                        y.append(B) }
                                    break;
                                case "spacer":
        ...
        function s(N, B, K, I) {
            ...
            if (typeof(J) === "object") {
                if ("prev" in J) { G.html(J.prev) }
                if ("next" in J) { H.html(J.next) } }
            var M = N.data("showPrevious");
            if (typeof(M) === "number" || M === false) { M = p(N.data("showPrevious"), true) }
            var L = $('<div class="calendar-month-navigation"></div>');
            L.attr("id", N.attr("id") + "_nav-prev");
            L.data("navigation", "prev");
            if (M !== false) { prevMonth = (I - 1);
                prevYear = K;
                if (prevMonth == -1) { prevYear = (prevYear - 1);
                    prevMonth = 11 }
                L.data("to", { year: prevYear, month: (prevMonth + 1) });
                L.append(G);
                if (typeof(N.data("actionNavFunction")) === "function") { L.click(N.data("actionNavFunction")) }
                L.click(function(P) { w(N, B, prevYear, prevMonth) }) }
            var F = N.data("showNext");
            if (typeof(F) === "number" || F === false) { F = p(N.data("showNext"), false) }
            var D = $('<div class="calendar-month-navigation"></div>');
            D.attr("id", N.attr("id") + "_nav-next");
            D.data("navigation", "next");
            if (F !== false) { nextMonth = (I + 1);
                nextYear = K;
                if (nextMonth == 12) { nextYear = (nextYear + 1);
                    nextMonth = 0 }
                D.data("to", { year: nextYear, month: (nextMonth + 1) });
                D.append(H);
                if (typeof(N.data("actionNavFunction")) === "function") { D.click(N.data("actionNavFunction")) }
        ...
        function e(B, D) {
            if (B.data("showDays") === true) {
                ...
                $(z).each(function(F, G) { A.append("<th>" + G + "</th>") });
                D.append(A) }
            return D }
        ...
        function q(G, F, I, N) {
            ...
            for (var A = 0; A < H; A++) {
                ...
                for (var K = 0; K < 7; K++) {
                    if (K < D || C > y) { z.append("<td></td>") } else {
                        var O = G.attr("id") + "_" + k(I, N, C);
                        var M = O + "_day";
                        var L = $('<div id="' + M + '" class="day" >' + C + "</div>");
                        L.data("day", C);
                        if (G.data("showToday") === true) {
                            if (x(I, N, C)) { L.html('<span class="badge badge-today">' + C + "</span>") } }
                        var J = $('<td id="' + O + '"></td>');
                        J.append(L);
                        J.data("date", k(I, N, C));
                        J.data("hasEvent", false);
                        if (typeof(G.data("actionFunction")) === "function") { J.addClass("dow-clickable");
                            J.click(function() { G.data("selectedDate", $(this).data("date")) });
        ...
        function u(z, y, C) {
            ...
            if (typeof(B) != "object" || typeof(B.url) == "undefined") { alert("Invalid calendar event settings");
                return false }
            var A = { year: y, month: (C + 1) };
            $.ajax({ type: "GET", url: B.url, data: A, dataType: "json" }).done(function(D) {
                var E = [];
                $.each(D, function(G, F) { E.push(D[G]) });
                z.data("events", E);
                f(z, "ajax") });
            return true }
        ...
        function f(B, A) {
            ...
            var y = B.data("events");
            if (y !== false) { $(y).each(function(H, J) {
                    var D = B.attr("id") + "_" + J.date;
                    var F = $("#" + D);
                    var K = $("#" + D + "_day");
                    F.data("hasEvent", true);
                    if (typeof(J.title) !== "undefined") { F.attr("title", J.title) }
                    if (typeof(J.classname) === "undefined") { F.addClass("event") } else { F.addClass("event-styled");
                    ...
                    if (typeof(J.body) !== "undefined") {
                        var I = false;
                        if (A === "json" && typeof(J.modal) !== "undefined" && J.modal === true) { I = true } else {
                            if (A === "ajax" && "modal" in C && C.modal === true) { I = true } }
                        if (I === true) { F.addClass("event-clickable");
                            var L = h(D, J.title, J.body, J.footer);
                            $("body").append(L);
                            $("#" + D).click(function() { $("#" + D + "_modal").modal() }) } } }) } }
...
$.fn.zabuto_calendar_defaults = function() {
    ...
    var c = a.getFullYear();
    var e = a.getMonth() + 1;
    var b = { language: false, year: c, month: e, show_previous: true, show_next: true, cell_border: false, today: false, show_days: true, weekstartson: 1, nav_icon: false, data: false, ajax: false, legend: false, action: false, action_nav: false };
    return b };
